...
def tag_motor(packet_handler, current_id, new_id):
    ...
    if result == COMM_SUCCESS and error == 0:
        ...
        # CRITICAL: Save the ID change to EEPROM
        print(f"Saving ID change to EEPROM...")
        time.sleep(0.1)  # Brief pause before save
        
        # Write to EEPROM save register (register 40)
        save_result, save_error = packet_handler.write1ByteTxRx(new_id, 40, 1)
        if save_result == COMM_SUCCESS and save_error == 0:
            print(f"ID change saved to EEPROM successfully")
        else:
            print(f"Warning: Failed to save ID to EEPROM. Error: {save_error}")
        
        # Wait for EEPROM write to complete
        time.sleep(0.5)
        
        # Lock EEPROM after saving
        lock_result, _ = packet_handler.LockEprom(new_id)
        if lock_result == COMM_SUCCESS:
            print(f"EEPROM locked for motor {new_id}.")
        else:
            print(f"Warning: Failed to lock EEPROM for motor {new_id}.")
            
        # Update local motor dictionary
        if current_id in motors:
            motors[new_id] = motors.pop(current_id)
            
        # Verify the change persisted
        print("Verifying ID change...")
        time.sleep(0.5)
        position, speed, verify_result, _ = packet_handler.ReadPosSpeed(new_id)
        if verify_result == COMM_SUCCESS:
            print(f"✓ Verification successful: Motor responds to ID {new_id}")
            print(f"⚠️  IMPORTANT: Power cycle the servo to test if ID persists!")
        else:
            print(f"✗ Verification failed: Motor may not respond to ID {new_id}")
            
    else:
        print(f"Failed to update ID from {current_id} to {new_id}. Error: {error}")
        # Try to lock EEPROM even if ID change failed
        packet_handler.LockEprom(current_id)
